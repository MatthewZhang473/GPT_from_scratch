...
def estimate_loss():
    out = {}
    # set to eval phase - doesn't need for now
    model.eval()
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        wei = q @ k.transpose(-1,-2) * head_size**-0.5 # (B, T, hs) @ (B, hs, T) -> (B, T, T)
        wei = wei.masked_fill(self.tril[:T, :T] == 0, float('-inf'))
        wei = F.softmax(wei, dim=-1)
        wei = self.dropout(wei) #randomly prevent nodes communicating
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size):
        ...
        self.dropout = nn.Dropout(dropout)
    def forward(self, x):
        # concacenate in the channel dimension
        out = torch.cat([h(x) for h in self.heads], dim=-1) # num_heads * (B,T,single_head_size) -> (B,T,head_size)
        out = self.dropout(self.proj(out))
        return out
        
class FeedForward(nn.Module):
    """a simple linear MLP followed by non-linearity"""
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.ln_f = nn.LayerNorm(n_embd) #final layer nrom
